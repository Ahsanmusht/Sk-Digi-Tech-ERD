...
const queryParamPromise = (sql, queryParam) => {
  return new Promise((resolve, reject) => {
    pool.query(sql, queryParam, (err, results) => {
      if (err) {
        reject(err);
      } else {
        resolve(results);
...
exports.postLogin = (req, res, next) => {
  try {
    ...
    pool.query(sql5, [email], async (err, results) => {
      if (
        results.length === 0 ||
...
exports.getDashboard = (req, res, next) => {
  ...
  pool.query(sql6, [req.user], (err, result) => {
    if (err) throw err;
...
exports.forgotPassword = async (req, res, next) => {
  ...
  pool.query(sql2, [token, email], (err, success) => {
    if (err) {
    ...
    if (resetLink) {
      jwt.verify(resetLink, process.env.RESET_PASSWORD_KEY, (err, data) => {
        if (err) {
          ...
          pool.query(sql1, [resetLink], async (err, results) => {
            if (err || results.length === 0) {
              ...
              const sql2 =
                ...
                pool.query(sql2, [hashed, resetLink], (errorData, retData) => {
                if (errorData) {
                  throw errorData;
                } else {
                  req.flash(
